...
{/breadcrumbs}
<h1><img src="{image}/icons/shoppingcart32.png" border="0" alt="" class="titleicon"/>[[Add New Product]]</h1>
			...
			{foreach from=$form_fields_info item=form_field}
				...
				{else}
					<tr {if $form_field.id == 'user_group_sid' || $form_field.id == 'listing_type_sid'}style="display:none;"{else}class="{cycle values = 'evenrow,oddrow'} {if in_array($form_field.id, array('listing_duration', 'number_of_listings', 'featured'))}post-listing-field{/if}"{/if}>
						<td>[[$form_field.caption]]</td>
						<td class="productInputReq">{if $form_field.is_required}*{/if}</td>
						<td><div  class="productInputField">{if $form_field.id == 'price'}{currencySign} {/if}{input property=$form_field.id} {if $form_field.id == 'listing_duration'}[[days]]{/if} {if $form_field.id == 'width' || $form_field.id == 'height'} [[pixels]]{/if}</div>{if $form_field.comment}<small>[[{$form_field.comment}]]</small>{/if}</td>
					</tr>
				{/if}
			{/foreach}
			</table>
