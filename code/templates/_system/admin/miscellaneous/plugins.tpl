{if $url == '/backfilling/'}
	{breadcrumbs}[[Job Backfilling]]{/breadcrumbs}
	<h1><img src="{image}/icons/plug32.png" border="0" alt="" class="titleicon"/>[[Job Backfilling]]</h1>
{else}
...
<form method="post">
	{foreach from=$groups item=plugins key=group}
		<div {if ($url == '/backfilling/' && $group != 'Job Backfilling') || ($url != '/backfilling/' && $group != 'Common')}style="display: none;"{/if}>
			<table>
				<thead>
					<tr>
						<th>[[Plugin Name]]</th>
				...
				<tbody>
					{foreach from=$plugins item=plugin key=key}
						<tr {if in_array($plugin.name, array('ApiPlugin', 'GooglePlusSocialPlugin', 'FacebookSocialPlugin'))}style="display: none;"{/if} class="{cycle values = 'evenrow,oddrow'}">
							<td>{$plugin.name}</td>
							<td>
...
<script>
    function pluginsGroup(group, pluginName)
    {
		{foreach from=$groups item=plugins key=group}
			{foreach from=$plugins item=plugin key=key}
				{if $plugin.group_id}
					if ("{$plugin.group_id}" == group ) {
						if ($("#" + group + "_" + pluginName).attr("checked") == true && "{$plugin.name}" != pluginName) {
