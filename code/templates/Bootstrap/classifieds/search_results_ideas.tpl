...
{description}[[ideas from $site_name]]{/description}

{capture name=search}
	{module name='classifieds' function='search_form' form_template='quick_search_ideas.tpl' listing_type_id='Idea' browse_request_data=$browse_request_data searchId=$searchId}
{/capture}
...
{else}
	{if $is_company_profile_page}
		{include file="search_results_ideas_profile.tpl"}
	{else}
		...
		<div class="container">
			<div class="details-body details-body__search {if ($refineSearch && $currentSearch) || ($refineSearch && $refineFields) || $user_page_uri}row{else}no-refine-search{/if}{if 'banner_right_side'|banner} with-banner{/if}">
				{if $smarty.request.not_found}
					<div class="col-xs-12">
						<div class="alert alert-info text-center">
							[[Sorry, that idea is no longer available. Here are some results that may be similar to the idea you were looking for.]]
						</div>
					</div>
				{/if}
						...
						{foreach from=$browse_navigation_elements item=element name="nav_elements"}
							<h1 class="search-results__title {if $user_page_uri}browse-by__title {else}col-sm-offset-3 col-xs-offset-0{/if}">
								{if $user_page_uri == '/categories/'}
									...
									[[$opps_number $category_name ideas]]
								{else}
									{assign var="location" value=$element.caption|escape}
									[[$opps_number ideas found in $location]]
								{/if}
							</h1>
						{/foreach}
					{else}
						<h1 class="search-results__title {if $user_page_uri}browse-by__title {else}col-sm-offset-3 col-xs-offset-0{/if}">
							[[$opps_number ideas found]]
						</h1>
					{/if}
					{if $listing_type_id != ''}
						<a class="btn create-job-alert btn__blue"
						   ...
						   data-title='[[Create Idea Alert]]'>
							[[Email me ideas like this]]
						</a>
					{/if}
				</div>
				...
				<div class="search-results {if ($refineSearch && $currentSearch) || ($refineSearch && $refineFields)}col-xs-12 col-sm-9{/if}{if $user_page_uri} search-results__small{/if}">
					{if $listings}
						{include file="search_results_ideas_listings.tpl"}
						<button type="button" class="load-more btn btn__white" data-page="2" data-backfilling="false" data-backfilling-page="1">
							[[Load more]]
						</button>
					{else}
						<div class="alert alert-danger no-listings-found hidden">
							[[Sorry, we don't currently have any ideas for this search. Please try another search.]]
						</div>
						<button type="button" class="load-more btn btn__white" data-page="2" data-backfilling="false" data-backfilling-page="1">
							[[Load more]]
						</button>
...
{if $GLOBALS.user_page_uri == '/ideas/' || $browse_request_data}
	{javascript}
