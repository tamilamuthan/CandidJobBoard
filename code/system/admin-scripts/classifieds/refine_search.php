	...
	public function execute()
	{
		...
		foreach ($listingTypes as $key => $listingType) {
			...
			foreach ($fields as $field_key => $field) {
				if ($field['type'] == 'location') {
					if (is_array($field['fields'])) {
						foreach ($field['fields'] as $subFieldKey => $subField) {
							if (!in_array($subField['id'], array('Country', 'City', 'State'))) {
				...
				if (!in_array($field['type'], array('list', 'multilist', 'string', 'boolean'))
														|| in_array($field['id'], array('ApplicationSettings', 'access_type'))) {
