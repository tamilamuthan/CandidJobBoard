...
class SJB_ObjectDBManager
{
			...
			elseif ($object_property->isParent()){
				...
				if ($childProperties) {
					foreach ($childProperties as $childProperty) {
						...
						if (empty($property_sql_value) && (in_array($childProperty->getType(), array('boolean', 'integer', 'float')) || in_array($childProperty->getId(), array('Latitude', 'Longitude')))) {
	...
	public static function getObjectInfo($dbTableName, $objectSid)
	{
		...
		foreach ($objectInfo as $key => $val) {
			$locationFields = array("{$key}_Country", "{$key}_State", "{$key}_City", "{$key}_ZipCode", "{$key}_Latitude", "{$key}_Longitude");
	...
	public static function saveLocationField($db_table_name, $field_table_name, $object, $oldFieldID = false)
	{
		...
		if (!$issetField) {
			...
			if ($oldFieldID) {
				...
				SJB_DB::query("ALTER TABLE `?w` CHANGE `?w` `?w` VARCHAR(255) NULL", $db_table_name, $oldFieldID."_Latitude", $fieldID."_Latitude");
				SJB_DB::query("ALTER TABLE `?w` CHANGE `?w` `?w` VARCHAR(255) NULL", $db_table_name, $oldFieldID."_Longitude", $fieldID."_Longitude");
			}
			else {
				SJB_DB::query("ALTER TABLE `?w` ADD `?w` ?w", $db_table_name, $fieldID, $fieldType);
				...
				SJB_DB::query("ALTER TABLE `?w` ADD `?w` VARCHAR(255) NULL", $db_table_name, $fieldID."_Latitude");
				SJB_DB::query("ALTER TABLE `?w` ADD INDEX ( `?w` ) ", $db_table_name, $fieldID."_Latitude");

				SJB_DB::query("ALTER TABLE `?w` ADD `?w` VARCHAR(255) NULL", $db_table_name, $fieldID."_Longitude");
				SJB_DB::query("ALTER TABLE `?w` ADD INDEX ( `?w` ) ", $db_table_name, $fieldID."_Longitude");
			}
		}
	}
