						...
						if (!empty($callback_data['item_id_'.$i])) {
							foreach ($items['products'] as $key => $product) {
								if ($product == $callback_data['item_id_' . $i]) {
									...
									$paymentHandler->createContract($user_sid, $invoice->getSID(), 'active');
								}
							}
						} else {
								...
								if ($product == -1) {
									...
									switch ($type) {
										case 'featuredListing':
											$paymentHandler->makeFeatured();
											break;
										case 'activateListing':
											$paymentHandler->activateListing();
											break;
									}
								}
							}
						...
						if ($product == -1) {
							...
							switch ($type) {
								case 'featured_listing':
									$paymentHandler->unmakeFeatured();
									break;
								case 'activate_listing':
									$paymentHandler->deactivateListing();
									break;
								default:
									break;
							}
